...
def continual_training_benchmark(
    ...
    if label_from_zero_each_task:      # task-IL
...
def fewshot_testing_benchmark(
    ...
    for exp_idx in range(n_experiences):
        ...
        classes_in_exp.append((np.arange(n_way)+class_offset).astype(np.int64))
        class_mapping = np.array([-1] * num_classes)
        class_mapping[selected_class_idxs] = np.arange(n_way)+class_offset
...
class Subset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return x, mapped_y, self.get_task_label(index)
...
if __name__ == "__main__":
    ...
    #     n_experiences=300, n_way=10, n_shot=10, n_query=10, mode='sys',
    #     task_offset=10,
    #     seed=1234, dataset_root='../../datasets',
    # )
    benchmark = fewshot_testing_benchmark(
        n_experiences=50, image_size=(224, 224),
        mode='sys',
        n_way=10, n_shot=10, n_val=5, n_query=10,
        task_offset=10, class_offset=30,       # task all as 10, class all as 30-39
        seed=1234,
        dataset_root='../../datasets')
